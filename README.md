...
골드가 모이면 마을 어귀에 **영지**를 개척할 수 있다. 한 시간마다 몬스터가
몰려오고, 성벽·화살탑·마법탑·병영이 자동으로 막는다(방치형 디펜스). 농장은
골드를 벌고, 대장간은 모험가를 돕는다. 격퇴 4회마다 수련 포인트를 얻어
공격/방어/경제 스탯을 올린다. 방치하면 성벽이 뚫려 골드를 약탈당하지만
손실에는 상한이 있어 언제든 재건할 수 있다.
...
- 틱마다 `mulberry32(hash32(seed, tick))`로 난수를 만들어 사건을 하나 뽑는다
  (이동 / 회수 / 조우 / 발굴 / 정비 / 관측)
- 플레이어 행위(방침 변경, 강화, 소포, 영지 개척·건설·수련)는 `{t, k, v}`로
  기록되어 해당 틱에 적용된다. 영지 습격은 개척 시점부터 12틱(1시간)마다
  같은 난수 규칙으로 재현된다
